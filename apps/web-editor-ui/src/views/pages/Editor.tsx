...
\\end{document}`,
  ...
  useEffect(() => {
    const checkMobile = () => {
      ...
      setIsSidebarVisible(!isMobileView); // Hide sidebar by default on mobile
    };
    checkMobile();
...
\\end{document}`;
  ...
  const handleFileSelect = (fileName: string) => {
    ...
    setContextMenu(null);
    if (isMobile) {
      setIsSidebarVisible(false); // Auto-hide sidebar on mobile after selection
    }
  };
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.name.endsWith('.tex')) {
      ...
      reader.onload = (e) => {
        ...
        if (isMobile) {
          setIsSidebarVisible(false); // Auto-hide sidebar on mobile after upload
        }
      };
  ...
  return (
    <div className="flex h-screen bg-gray-50" onClick={handleClickOutside}>
      ...
      <div className="flex flex-row h-full w-full">
        {/* Sidebar */}
        <div
          className={`bg-gray-800 text-white flex flex-col shadow-xl transition-transform duration-300 ${
            isSidebarVisible
              ? 'w-64 translate-x-0'
              : 'w-0 -translate-x-full overflow-hidden'
          } ${isMobile ? 'fixed top-0 left-0 h-full z-20' : 'relative'}`}
        >
          ...
          <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                title={
                  isSidebarVisible ? 'Masquer le menu' : 'Afficher le menu'
                }
              >
                <Menu className="w-5 h-5 text-gray-500" />
              </button>
            ...
            <div className="flex items-center space-x-2">
              {/* <button
                onClick={handleToggleSidebar}
                className="p-2 rounded-lg hover:bg-gray-100 hidden md:block"
                title={isSidebarVisible ? 'Masquer la barre latérale' : 'Afficher la barre latérale'}
              >
                <Menu className="w-5 h-5 text-gray-500" />
              </button> */}
